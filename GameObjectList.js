export class Node {
    constructor(gameObject, next = null){
        this.data = gameObject;
        this.next = next;
    }
}
export class GameObjectLinkedList {
    constructor() {
    ...
    drawListData(ctx) {
      ...
      while (current) {
        let object = current.data;
        ctx.beginPath();
        ctx.arc(object.position.x, object.position.y, object.size.radius, 0, Math.PI*2);
        ctx.stroke();
        current = current.next;
      }
    }
